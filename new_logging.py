...
with open(filename, 'w') as f:
    ...
    f.write("Elapsed Time, Elapsed Steps, Motor 2 Angle\n")

# Flag to control logging
logging_active = False

# Record the start time and step count
start_time = None
elapsed_steps = 0

def log_angles_continuously():
    """Logs Motor 2's angle at quarter-second intervals."""
    global logging_active, start_time, elapsed_steps
    while logging_active:
        elapsed_time = round(time() - start_time, 2)  # Calculate elapsed time since start
        with open(filename, 'a') as f:
            f.write(f"{elapsed_time}, {elapsed_steps}, {motor_positions[2]:.3f}\n")
        sleep(0.125)  # Log every quarter second

def move_motors_by_angle(angle_difference):
    global motor_positions, logging_active, start_time, elapsed_steps
    ...
    elapsed_steps = 0  # Initialize step count
    ...
    for step in range(abs(steps_needed_2)):
        ...
        # Increment the elapsed steps after each motor step
        elapsed_steps += 1
